...
<head>
    ...
    <script type="text/javascript">
        function alertInfo()
        {
            ...
            var regResult = "${requestScope.regResult}";//注册结果信息
...
<div class="login-container">
    <form class="login-form" action="${path}user/clientLogin.action" method="post" enctype="multipart/form-data">
        ...
        <div class="input-group">
            <input type="password" id="upass" name="upass" class="input-field">
            <label for="upass" class="input-label">
                <span class="label-title">密码</span>
            </label>
        </div>
        <a href="${path}FileUploadDemo.jsp">文件上传测试</a>

        <button type="submit" class="login-button">登录<i class="ai ai-enter"></i></button>

        <a href="${path}a/clientReg.action"}>
                    <button type="button" class="login-button">注册<i class="ai ai-enter"></i></button>
        </a>

        <a href="${path}a/backLogin.action"}>
            <button type="button" class="login-button">后台登录入口<i class="ai ai-enter"></i></button>
        </a>
    </form>
</div>
</body>
